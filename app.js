...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = `${$('.product-details-desc > h2').text().trim()} ${$('.product-details-desc > h3').text().trim()} ${'برند آمیتیس'}` ;
          data["category"] = $('.product-details-desc > h2').last().length
               ? $('.product-details-desc > h2').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'آمیتیس';
          ...
          let specification = {};
          const rowElements = $('.order-table > table > tbody > tr')
          for (let i = 0; i < rowElements.length; i++) {
               const row = rowElements[i];
               const key = $(row).find('> td:first-child').text()?.trim()
               const value = $(row).find('> td:not(:first-child) > span').map((i, p) => $(p)?.text()?.trim()).get().join('-');
          ...
          // descriptionString
          const descriptionString = $('.product-details-desc > p')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
          ...
          // Download Images
          let imagesUrls = $('.product-details-image img, .product-details-image img')
               .map((i, img) => 'https://www.amitis.org' + $(img).attr("src").replace(/(-[0-9]+x[0-9]+)/g, "")).get();
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/amitis");
