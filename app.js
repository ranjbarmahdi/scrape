...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          let title = $('h1.product_title').length ? $('h1.product_title').text().trim() : "";
          let names = [];
          ...
          const variantArrays = variants.map(variant => {
               const liElements = $(variant).find('td > div > div > span').get();
          ...
          for (const combination of combinations) {
               names.push(`${title} ${combination.join(' ')} ${'برند پنتا'}`);
          }
          ...
          for(const name of names){
               ...
               data["category"] = $('.posted_in > a').first().length
                    ...
                    : "";
               data["brand"] = $('notFound').text()?.trim() || 'پنتا';
               ...
               // price_1
               const xpaths = [
                    '/html/body/div[3]/div[2]/div/div/div/div[2]/div/div[2]/div[3]/div[1]/div[2]/span/bdi/text()'
               ];
               const mainXpath = '/html/body/div[3]/div[2]/div/div/div/div[2]/div/div[2]/div[3]/div[1]/div[2]/span/bdi/text()';
               if (xpaths.length) {
                    // Find Price
               ...
               let specification = {};
               const rowElements = $('.shop_attributes table > tbody > tr')
               ...
               // descriptionString
               const descriptionString = $('#tab-description > p')
                    .filter((i, e) => {
                         return $(e).text()?.trim()
                    })
                    .map((i, e) => $(e).text()?.trim())
               ...
               // Download Images
               let imagesUrls = $('.single_p_gallery img')
                    .map((i, img) => $(img).attr("src")?.replace(/(-[0-9]+x[0-9]+)/g, ""))
                    .get();
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/pantalift");
