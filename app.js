...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('h1.product_title').length ? `${$('h1.product_title').text().trim()} ${'برند هانتا'}` : "";
          data["category"] = $('notFound').last().length
               ? $('notFound').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'هانتا';
          ...
          for (let i = 0; i < rowElements.length; i++) {
               ...
               const value = $(row).find('> td > p > span').map((i, p) => $(p)?.text()?.trim()).get().join('-');
          ...
          // descriptionString
          const descriptionString = $('#tab-description > div > ul > li')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/hantaibms");
