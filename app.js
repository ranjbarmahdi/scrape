...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('h1').length ? `${$('h1').text().trim() } ${'برند گسکول'}`: "";
          ...
          // descriptionString
          const descriptionString = $('.attachment-full.size-full')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('/n');
          ...
          // Download Images
          let imagesUrls = $('.attachment-full.size-full')
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/gescool");
