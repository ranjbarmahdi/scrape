...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
    try {
        ...
        const data = {};
        data["title"] = $("h1.services_title").length
            ? `${$("h1.services_title").text().trim()} ${"برند آداسا"}`
            : "";
        ...
        data["brand"] = $("notFound").text()?.trim() || "آداسا";
        ...
        // Download Images
        const image_xpaths = [
            "/html/body/div[1]/section[2]/article/div/div/div/div[1]/div[1]//img",
        ];
