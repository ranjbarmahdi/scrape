...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('.product-in-text > h2').length ? `${$('.product-in-text > h2').text().trim()} ${'برند پرشیا سازه استیل'}` : "";
          data["category"] = $('notFound').last().length
               ? $('notFound').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'پرشیا سازه استیل';
          ...
          // descriptionString
          const descriptionString = $('.product-in-text')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
          ...
          // Download Images
          let imagesUrls = $('.gallery img')
               .map((i, img) => 'https://pssteel.ir' + $(img).attr("src").replace(/(-[0-9]+x[0-9]+)/g, "")).get();
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/pssteel");
