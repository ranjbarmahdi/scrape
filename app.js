...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
    try {
        ...
        data["title"] = $("h1").length
            ? `${"سنگ سایز"} ${$("h1").text().trim()} ${"جوشقان برند ایران مرمریت سپاهان"}`
            : "";
        ...
        data["brand"] = $("notFound").text()?.trim() || "ایرن مرمریت سپاهان";
        ...
        // descriptionString
        const descriptionString = $(".entry-content > p:first")
            .map((i, e) => $(e).text()?.trim())
            .get()
            .join("\n");
        ...
        // Download Images
        const image_xpaths = ['//div[@class="post-image-wrap"]//img'];
