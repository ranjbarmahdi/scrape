...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('h2.product_title').length ? $('h2.product_title').text().trim() : "";
          data["category"] = $('span.posted_in > a').last().length
               ? $('span.posted_in > a').last()
          ...
          let specification = {};
          const rowElements = $('div.tab-content table > tbody > tr')
          for (let i = 0; i < rowElements.length; i++) {
               const row = rowElements[i];
               const value = $(row).find('> td:first-child').text()?.trim()
               const key = $(row).find('> td:last-child').map((i, p) => $(p)?.text()?.trim()).get().join('-');
          ...
          // descriptionString
          const descriptionString = $('div.description > ul > li')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
          ...
          // Download Images
          let imagesUrls = $('.product-summary-wrap img.lazyloaded')
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/ashekarsazco");
