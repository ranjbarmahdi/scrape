...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          do {
               nextPageBtn = await page.$$('.esg-navigationbutton.esg-loadmore');
          ...
          const title = $('h1.title').length ? $('h1.title').text().trim() : "";
          
          const products = $('.mainul > li > div')
          for (const product of products){
               try {
                    ...
                    // Download Images
                    let imagesUrls = $(product).find('>div:first-child > div > img')
                    ...
                    console.log("Error in scrapSingleProduct foor loop");
               }
          }
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/semnanTile");
