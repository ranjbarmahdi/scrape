...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('h1.product_title ').length ? `${$('h1.product_title ').text().trim()} ${'برند نماچین'}` : "";
          data["category"] = $('.product_meta a').last().length
               ? $('.product_meta a').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'نماچین';
          ...
          // Download Images
          let imagesUrls = $('.theiaStickySidebar img')
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/namachin");
