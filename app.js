...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('.wpb_wrapper > h2').length ? `${$('.wpb_wrapper > h4').text().trim()} ${$('.wpb_wrapper > h2').text().trim()}` : "";
          data["category"] = $('notFound').last().length
               ? $('notFound').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'درب راد';
          ...
          // descriptionString
          const descriptionString = $('.wpb_wrapper > p')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/aradchoobco");
