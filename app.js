...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          const data = {};
          data["title"] = $('h2.title.mjs-product-title').length ? $('h2.title.mjs-product-title').text().trim() : "";
          data["category"] = $('.breadcrumbs > li > a').last().length
               ? $('.breadcrumbs > li > a').last()
                    .map((i, a) => $(a).text().trim()).get().join(" > ")
               : "";

          data["brand"] = $('notFound').text()?.trim() || 'آرین تکنیک';
          ...
          let specification = {};
          const rowElements = $('.mjstable > tbody > tr')
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/arianteknik");
