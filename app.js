...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          data["brand"] = $('notFound').text()?.trim() || 'پشم سنگ ایران';
          ...
          let specification = {};
          const rowElements = $('.woocommerce-product-details__short-description > li').filter((i, e) => {
               return $(e).text()?.trim() && $(e).text()?.includes(':')
          }).get();
          ...
          for (let i = 0; i < rowElements.length; i++) {
               ...
               const value = rowString.split(':')?.slice(1)?.map(t => t?.trim())?.join('-');
          ...
          // descriptionString
          const descriptionString = $('#tab-description li')
               .map((i, e) => $(e).text()?.trim())
               .get()
               .join('\n');
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/pashmesangiran");
