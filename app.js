...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          if (offPercent.length) {
               ...
               data["price"] = $('.woocommerce-Price-amount.amount > bdi').first().text().replace(/[^\u06F0-\u06F90-9]/g, "");
               data["xpath"] = '/html/body/div[1]/div[2]/div/div[2]/div/section[2]/div/div[2]/div/section[1]/div/div[2]/div/div/div/div/a/span/span/ins/span/bdi/text()';
          }
          ...
          if('برند کاغذدیواری' in specification){
               data['brand'] = specification['برند کاغذدیواری']
          }
          // descriptionString
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/newwall");
