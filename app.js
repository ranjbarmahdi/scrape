...
async function scrapSingleProduct(page, productURL, imagesDIR, documentsDir, rowNumber = 1) {
     try {
          ...
          do {
               ...
               const products = $('#caselist ul > li')
               for (const product of products){
                    try {
                         const data = {};
                         data["title"] = $(product).find('span.title').length ? $(product).find('span.title').text().trim() : "";
                         data["category"] = $('notFound').last().length
                              ? $('notFound').last()
                                   .map((i, a) => $(a).text().trim()).get().join(" > ")
                              : "";
               
                         data["brand"] = $('اینسایز').text()?.trim() || '';
                         ...
                         // Download Images
                         let imagesUrls = $(product).find('.imgwh img') 
                              .map((i, img) => 'http://insize.ir' + $(img).attr("src").replace(/(-[0-9]+x[0-9]+)/g, "")).get();
                         ...
                         console.log("Error in while => for lop : ", error);
                    }
               }
               
               nextPageText = $('.k_pagelist > a:last').text()
               console.log("nextPageText :",nextPageText);
               nextPageBtn = await page.$$('.k_pagelist > a');
               if (nextPageBtn.length && nextPageText?.includes('Next')) {
                   let btn = nextPageBtn.at(-1);
                   await btn.click();
               }
               await delay(3000);
          }
          while (nextPageBtn.length && nextPageText?.includes('Next'))
...
async function main() {
     ...
     let page;
     try {
          const DATA_DIR = path.normalize(__dirname + "/insize");
