...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = [
            "https://san-marking.com/product-category/%d8%ae%d8%af%d9%85%d8%a7%d8%aa/",
            "https://san-marking.com/product-category/%d8%ac%d8%aa-%d9%be%d8%b1%db%8c%d9%86%d8%aa%d8%b1/",
        ];
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $(".woocommerce-LoopProduct-link")
                    .map((i, e) => $(e).attr("href"))
                    .get();
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ["https://san-marking.com/"];
