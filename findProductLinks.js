    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $('.jet-listing-grid__item .elementor-heading-title > a')
                    .map((i, e) => $(e).attr('href'))
                    .get()
                ...
                nextPageBtn = await page.$$('.jet-filters-pagination__item.prev-next.next')
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ['https://azaretesal.com/shop/']
