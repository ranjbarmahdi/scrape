...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = [
            "https://www.alvanpaint.com/allProduct/11/fa",
        ];
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $(".product-grid .img-product > a")
                    .map((i, e) => "https://www.alvanpaint.com" + $(e).attr("href"))
                    .get();
                ...
                let x = false;
                const nextPage = $(".pagination > li").filter((i, e) => {
                    return $(e).text().includes("›");
                });
                if (nextPage.length) {
                    const aTag = $(nextPage[0])?.find(">a");
                    if (aTag.length) {
                        x = true;
                    }
                }
                nextPageBtn = await page.$$(".pagination > li:last-child");
                if (x) {
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ["https://www.alvanpaint.com/fa"];
