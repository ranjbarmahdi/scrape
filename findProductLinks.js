...
async function findAllPagesLinks(page, mainLinks) {
    ...
    for (let i = 0; i < mainLinks.length; i++) {
        try {
            ...
            if (true) {
                let lsatPageNumber = 29;
                console.log("Last Page Number : ", lsatPageNumber);
                for (let j = 1; j <= lsatPageNumber; j++) {
                    const newUrl = `https://parsaray.com/%d9%81%d8%b1%d9%88%d8%b4%da%af%d8%a7%d9%87-%d9%be%da%a9-%d9%88%d9%88%d8%af/page/${j}/`;
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $(".products  div.product  .woocommerce-LoopProduct-link")
                    .map((i, e) => $(e).attr("href"))
                    .get();

                // insert prooduct links to un`visited
...
async function main() {
    try {
        const INITIAL_PAGE_URL = [
            "https://parsaray.com/%d9%81%d8%b1%d9%88%d8%b4%da%af%d8%a7%d9%87-%d9%be%da%a9-%d9%88%d9%88%d8%af/page/2/",
        ];
