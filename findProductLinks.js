...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = $("#menu-categories > li")
            .map((i, e) => {
                const inner = $(e)
                    .find("div > div > ul > li > a")
                    .map((i, w) => {
                        return $(w).attr("href")?.trim();
                    })
                    .get(); // Convert jQuery object to array

                if (inner.length) {
                    return inner; // Return the array directly
                } else {
                    const outer = $(e)
                        .find(">a")
                        .map((i, w) => {
                    ...
                    return outer; // Return the array directly
                }
            })
            .get(); // Flatten the result and convert to array
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ["https://tsaco-diesel.com/"];
