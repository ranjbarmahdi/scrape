...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = $("#navbar-112671516 > ul > li:nth-child(2) > ul > li > a")
            .map((i, a) => "http://www.ramaplastic.ir" + $(a).attr("href")?.trim())
            .get();
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $("h3.store-product-title > a")
                    .map((i, e) => "http://www.ramaplastic.ir" + $(e).attr("href"))
                    .get();
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ["http://www.ramaplastic.ir/"];
