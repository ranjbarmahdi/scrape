...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = $('.wd-sub-menu.sub-menu.color-scheme-dark > li > a')
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $('.product-element-bottom > .wd-entities-title > a')
                    .map((i, e) => $(e).attr('href'))
                    .get()
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ['https://roshashop.com/']
