...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = [
            "https://farpood.com/fa/product-category/%d9%81%d9%84%d8%a7%d8%b4%d8%aa%d8%a7%d9%86%da%a9/",
            "https://farpood.com/fa/product-category/%d8%ac%d8%a7%d9%85%d8%a7%db%8c%d8%b9/",
        ];
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $(".product  > a.woocommerce-loop-product__link")
                    .map((i, e) => $(e).attr("href"))
                    .get();
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ["https://farpood.com/fa/"];
